#!/bin/sh
# Install this local fork's Sol Advisor custom-agent templates without changing Codex config.

set -eu

usage() {
  cat <<'EOF'
Usage: install-sol-advisor.sh [--target-dir <path>] [--check]

Install the three local Sol Advisor custom-agent templates into the target directory.
...
if [ "$check_only" -eq 1 ]; then
  printf '%s\n' "CHECK PASSED: all local Sol Advisor agent files exactly match $template_dir."
  exit 0
fi
...
printf '%s\n' "INSTALL PASSED: all local Sol Advisor agent files exactly match $template_dir."
